...
- **The World Map modal is fully derived from the Forest screen manifest.** The M-key overworld map (`OverworldMapModal`) no longer carries hardcoded node positions or edges; it reads every node's grid cell from `FOREST_SCREENS` coords (`col = x`, `row = −y`) and derives its edge set from the screens' reciprocal exits. Display-only metadata (short labels, boss-tier glyphs) lives in `client/src/objects/world/forestMeta.ts`. The reason: a hardcoded mirror of `forest.ts` could drift independently after any graph change — deriving the modal makes the manifest the single source of truth. Adding a Forest screen now requires one entry in `shared/world/forest.ts` (with a `coord`) and one label/metadata entry in `forestMeta.ts`; the only remaining hardcoded positions are the isolated Hidden Alcove (teleport-only, no coord), the Swamp biome's entry node, and the Snow biome's entry node (neither is a Forest screen — both are static hardcoded biome nodes placed adjacent to their respective gate screens). The modal fit-scales the derived graph to the viewport on open and supports optional zoom/pan (with bounds clamping) to inspect detail — so the map is always fully visible regardless of how many screens the manifest grows to.
- **The Snow Fields biome node now appears on the World Map.** `snow_entry` is a static hardcoded node on the World Map, placed one grid step north of `forest_snow_gate`, connected by a biome-type edge (green). The modal derives Forest nodes from the manifest; Snow and Swamp are additional hardcoded biome nodes. The Snow node uses a distinct icy-blue colour to distinguish it from both Forest danger tiers and the Swamp's teal-green.
