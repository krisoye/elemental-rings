...
**Response (error):** 400 — `"ringId1 and ringId2 must be non-empty strings and shrineId must be a non-empty string"` | 400 — `"Ring not found or not owned"` | 400 — `"Cannot merge a ring with itself"` | 400 — `"Ring is locked in a duel (escrowed)"` | 400 — `"Ring is the pending WON ring"` | 400 — `"Rings must be the same element to merge"` | 400 — `"Shrine is sealed or not found"` | 401 — not authenticated
