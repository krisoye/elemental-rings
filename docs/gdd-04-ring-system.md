...
- **Swap-first ring management:** any two rings may be exchanged at any time regardless of pool fullness — a swap moves one ring out of a pool for every ring entering it, so capacity is never exceeded. Capacity limits apply only when *adding* a ring to a pool (e.g. carrying a Reliquary ring to the bench); they never apply to exchanging two rings already in the system. Escrowed (staked) rings and self-swaps are still rejected.
