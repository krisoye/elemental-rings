...
// #485/#491/#499 — broadcast by the server when a charge hold BEGINS (handleChargeStart).
...
// `startAngle` is always 0 (the orb starts aimed at the opponent — #499).
export interface ChargeOrbStartPayload {
  attackerId: string;
  slot: AttackSlot;
  startTime: number; // server wall-clock ms at chargeStart (for deterministic replay)
  startAngle: number; // always 0 (locked: orb starts aimed at opponent)
}
